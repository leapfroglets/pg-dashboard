...
class Insert extends Component{
  ...
  componentWillMount(){
    let data = {
      query:`select column_name,udt_name,character_maximum_length from information_schema.columns where table_name='${this.props.table}'`,
  ...
  insertData(){
    ...
    this.state.fields.map(field =>{
      ...
      value=(this.state.textBoxValue[field.column_name]) ? this.state.textBoxValue[field.column_name] :'null';  
      values= (value.match(regex)||value==='null') ? (values+value+','):(values+`'`+value+`'`+',');
    ...
    .then(response =>{
      this.setState({textBoxValue:emptyObject, reply:response.data.reply, error:''}) 
    })
    .catch(err =>{
            ...
            this.state.fields.map(field => {
              return(
                <tr key={field.column_name}>
                  ...
                  <td>{field.udt_name}({field.character_maximum_length})</td>
                  <td>
                    <input type ="text" className="form-control" placeholder="Value"
                    value={(this.state.textBoxValue[field.column_name]) ? this.state.textBoxValue[field.column_name]:""} onChange={(e) => {
          ...
          <tr>
            ...
            <td>
              <input type="button" className="btn btn-round btn-default"
                value="go" onClick = {() => {this.insertData()}}
              />
            </td>
          </tr>
          </tbody>
        </table>
        {this.state.reply}{this.state.error}
      </div>
    );
  }
}
